    ...
    if (snapshot.empty) {
      container.innerHTML = `
        <div class="empty-state">
          ...
          <a href="/patron/browsestalls.html" class="redeem-btn" style="margin-top: 20px; display: inline-block; text-decoration: none;">Browse Stalls</a>
        </div>
      `;
      return;
    }
...
document.getElementById("likes-container").addEventListener("click", async (e) => {
  ...
  if (e.target.closest(".view-menu-btn")) {
    ...
    if (stallId) {
      window.location.href = `/patron/menus.html?id=${stallId}`;
    }
  }
});
