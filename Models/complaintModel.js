...
// complaint was made against). Both filters are optional - pass null/undefined
// to skip either one.
...
// Updates status (and assigns the acting officer as the handler) and, if a
// note was given, appends it as a new ComplaintNotes row - both inside one
// transaction so a note is never saved against a status change that failed.
// Returns null if the complaint doesn't exist.
async function updateComplaintStatus(complaintId, officerId, status, note) {
  let connection;
  let transaction;
  ...
  try {
    ...
    updateRequest.input("officer_id", sql.Int, officerId);
    ...
    if (note) {
      ...
      noteRequest.input("officer_id", sql.Int, officerId);
